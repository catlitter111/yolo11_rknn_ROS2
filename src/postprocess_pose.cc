...
static void extract_keypoints_from_output(rknn_app_context_t *app_ctx, 
                                         rknn_output *outputs, 
                                         int keypoints_index, 
                                         letterbox_t *letter_box,
                                         float keypoints[17][3]) {
    // 安全检查
    if (keypoints_index < 0 || keypoints_index >= 8400) {
        printf("Warning: Invalid keypoints_index %d, using placeholder values\n", keypoints_index);
        for (int j = 0; j < 17; ++j) {
            keypoints[j][0] = 0.0f;
            keypoints[j][1] = 0.0f;
            keypoints[j][2] = 0.0f;
    ...
    printf("Debug: Extracting keypoints for index %d\n", keypoints_index);
    printf("Debug: Model quantized: %s\n", app_ctx->is_quant ? "Yes" : "No");
    printf("Debug: Output tensor type: %d\n", app_ctx->output_attrs[3].type);

    // 根据原始代码提取关键点数据
    for (int j = 0; j < 17; ++j) {
        if (app_ctx->is_quant) {
            #ifdef RKNPU1
            // RKNPU1 使用 uint8_t 数据
            keypoints[j][0] = (deqnt_affine_u8_to_f32(((uint8_t *)outputs[3].buf)[j * 3 * 8400 + 0 * 8400 + keypoints_index],
                    app_ctx->output_attrs[3].zp, app_ctx->output_attrs[3].scale) - letter_box->x_pad) / letter_box->scale;
            keypoints[j][1] = (deqnt_affine_u8_to_f32(((uint8_t *)outputs[3].buf)[j * 3 * 8400 + 1 * 8400 + keypoints_index],
                    app_ctx->output_attrs[3].zp, app_ctx->output_attrs[3].scale) - letter_box->y_pad) / letter_box->scale;
            keypoints[j][2] = deqnt_affine_u8_to_f32(((uint8_t *)outputs[3].buf)[j * 3 * 8400 + 2 * 8400 + keypoints_index],
                    app_ctx->output_attrs[3].zp, app_ctx->output_attrs[3].scale);
                                      #else
             // RKNPU2 使用 float16 数据，正确转换为 float
             keypoints[j][0] = ((float)((rknpu2::float16 *)outputs[3].buf)[j*3*8400+0*8400+keypoints_index] 
                                                             - letter_box->x_pad) / letter_box->scale;
             keypoints[j][1] = ((float)((rknpu2::float16 *)outputs[3].buf)[j*3*8400+1*8400+keypoints_index] 
                                                                 - letter_box->y_pad) / letter_box->scale;
             keypoints[j][2] = (float)((rknpu2::float16 *)outputs[3].buf)[j*3*8400+2*8400+keypoints_index];
             #endif
        } else {
            // 非量化模式，使用float数据
            keypoints[j][0] = (((float *)outputs[3].buf)[j*3*8400+0*8400+keypoints_index] 
                                                            - letter_box->x_pad) / letter_box->scale;
            keypoints[j][1] = (((float *)outputs[3].buf)[j*3*8400+1*8400+keypoints_index] 
                                                                - letter_box->y_pad) / letter_box->scale;
            keypoints[j][2] = ((float *)outputs[3].buf)[j*3*8400+2*8400+keypoints_index];
        }

        printf("Debug: Keypoint %d: x=%.2f, y=%.2f, conf=%.3f\n", 
               j, keypoints[j][0], keypoints[j][1], keypoints[j][2]);
    }
}
...
pose_object_detect_result_list *od_results) {
    ...
    for (int i = 0; i < validCount; ++i) {
        ...
        // 使用新的安全关键点提取函数
        extract_keypoints_from_output(app_ctx, _outputs, keypoints_index, letter_box, 
                                    od_results->results[last_count].keypoints);
