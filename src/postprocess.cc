...
{
    // 标签已硬编码，无需从文件加载
    printf("Using hardcoded clothing labels (13 classes)\n");
    return 0;
}
...
char *clothing_cls_to_name(int cls_id)
{
    ...
    {
        printf("Debug: cls_id >= OBJ_CLASS_NUM, returning null\n");
        return "null";
    }
    
    if (cls_id < 0)
    {
        printf("Debug: cls_id < 0, returning null\n");
        return "null";
    }
...
{
    // 标签已硬编码为常量字符串，无需释放内存
    printf("Cleanup post process (hardcoded labels, no memory to free)\n");
}
